...
def build_output_video(frames_dir, outframes_dir, best_matches, framerate, output_path, frame_multiply):
    print("building output video")
        
    for i, match_num in enumerate(best_matches):
        for j in range(int(frame_multiply)):
            
            shutil.copy(frames_dir / f'frame{int(frame_multiply)*match_num+j:06d}.png', outframes_dir / f'frame{int(frame_multiply)*i+j:06d}.png')
    subprocess.run(
        [
            'ffmpeg',
            '-hide_banner',
            '-loglevel', 'error',
            '-y',
            '-framerate', str(framerate*frame_multiply),
...
def process(carrier_path, modulator_path, output_path, default_frame_length):
    ...
    if 'video' in carrier_type:
        ...
        true_frame_length = carrier_duration / carrier_framecount
        frame_multiply = np.ceil(default_frame_length / true_frame_length)
        if frame_multiply == 0:
            frame_multiply = 1
        frame_length = frame_multiply * true_frame_length
        print("Video frame length of", true_frame_length, "will be multiplied by", frame_multiply)


    elif 'audio' in carrier_type:
        carrier_is_video = False
        if default_frame_length < .008:
            frame_length = 1/25
        else:
    ...
    if carrier_is_video:
        ...
        outframes_dir.mkdir()
        build_output_video(frames_dir, outframes_dir, best_matches, 1/frame_length, output_path, frame_multiply)
    else:
        subprocess.run(
            [
                'ffmpeg',
...
def main():
    ...
    parser.add_argument('default_frame_length', type=float, metavar='default_length', help='length of the divisions')
